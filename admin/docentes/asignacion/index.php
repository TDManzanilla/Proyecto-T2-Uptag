...
<script>
    ...
    function cargarMaterias(seccion) {
        ...
        fetch('../../../app/controllers/docentes/asignaciones/listado_materias_por_seccion.php?seccion=' + encodeURIComponent(seccion))
            ...
            .then(data => {
                ...
                data.materias.forEach(asignacion => {
                    const fila = document.createElement('tr');
                    fila.innerHTML = `
                        <td>${asignacion.materia}</td>
                        <td>${asignacion.docente}</td>
                        <td>
                            <button class="btn btn-primary btn-sm btn-asignar-docente" data-materia="${asignacion.materia}" data-seccion="${seccion}">
                                Asignar Docente
                            </button>
                        </td>
                    `;
    ...
    function confirmarAsignacionDocente(docenteId, materia, seccion) {
        fetch('../../../app/controllers/docentes/asignaciones/asignar_docente.php', {
            ...
            .then(data => {
                if (data.success) {
                    Swal.fire('Asignado', `El docente fue asignado correctamente.`, 'success');
